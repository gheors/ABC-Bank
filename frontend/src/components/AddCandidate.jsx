...
export default function AddCandidate({
  ...
  function modifyCandidate() {
    if (
      ...
      updateCandidate(editCandidate.id, data).then((res) => {
        setFirstName("");
        setLastName("");
        setAddress("");
        setBirthDate("");
        ...
        setCandidates(
          candidates.map((el) => {
            if (el.id === editCandidate.id) return res.data;
            else return el;
          })
        );
    ...
    <Flex flexDir={"column"} w={"350px"}>
      ...
      <FormControl pl={5} pr={5}>
        ...
        {phoneNumbers.map((num, index) => {
          return (
            <Flex justifyContent="center" key={`${num + index}`}>
              <Input
        ...
        <Button onClick={() => setPhoneNumbers([...phoneNumbers, ""])}>
          Add phone number
        </Button>
        ...
        {!editCandidate && (
          <Button w={"100%"} mt={5} onClick={() => addCandidate()}>
            Confirm candidate registration
          </Button>
        )}

        {editCandidate && (
          <Button w={"100%"} mt={5} onClick={() => modifyCandidate()}>
